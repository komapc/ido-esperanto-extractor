  ...
  "entries": [
    {
      "lemma": "vorto",
      "pos": "noun",
      "translations": {"io": ["vorto"]},
      "morphology": {"paradigm": "o__n"},
      "source_page": "https://eo.wiktionary.org/wiki/vorto"
    }
  ]
}
"""
...
def convert_to_standardized_format(old_format_data, dump_file, script_path):
    ...
    for entry_data in entries_list:
        ...
        entry = {
            ...
            "source_page": f"https://eo.wiktionary.org/wiki/{lemma}"
        }
...
def main(argv):
    ...
    ap.add_argument(
        "--output",
        type=Path,
        default=Path("sources/source_eo_wiktionary.json"),
    ...
    if args.dump:
        ...
        candidates = list(dumps_dir.glob("eowiktionary-*.xml.bz2"))
        if not candidates:
            # Fallback to old location
            dump_file = Path(__file__).parent.parent / "eowiktionary-latest-pages-articles.xml.bz2"
            if not dump_file.exists():
                # Try data directory
                dump_file = Path(__file__).parent.parent / "data" / "raw" / "eowiktionary-latest-pages-articles.xml.bz2"
        else:
            # Use most recent
    ...
    print(f"📖 Parsing Esperanto Wiktionary")
    ...
    try:
        # Parse using existing logic (EO→IO pairs) with optimizations
        ...
        print(f"   With IO translations: {stats['with_translations']:,}")
