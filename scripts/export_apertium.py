...
def _load_eo_generatable_lemmas(dix_path: Path = _EO_EPO_DIX) -> Optional[set]:
    """Casefolded set of every single-word lemma apertium-epo's monodix can
    generate — the ido→epo bidix winner gate (Fix B): a candidate translation
    that isn't in this set can't be generated by lt-proc -g regardless of how
    well-sourced it is (e.g. bert_embeddings leaking inflected surface forms
    like `trovis`/`troveblas`, or io_wiktionary glossing a word with an
    Esperanto term that was never a real headword, like `altetaksi`).

    Returns None (meaning "skip the gate") when the monodix is unavailable,
    matching _load_eo_vbser_lemmas' optional-input policy — a missing sibling
    checkout must not block the export, just weaken this one check.
    """
    if not dix_path.exists():
        logging.warning("apertium-epo monodix not found at %s — skipping the "
                        "EO-generatability bidix gate.", dix_path)
        return None
    try:
        root = ET.parse(dix_path).getroot()
    except ET.ParseError as e:
        logging.warning("Could not parse %s (%s) — skipping the EO-generatability "
                        "bidix gate.", dix_path, e)
        return None
    # Multi-word lm values (e.g. "tie ĉi", "la ilia") are kept, NOT excluded:
    # a multi-word <e lm="..."> is a genuine generatable phrase (its <i> stem
    # uses <b/> between components), unlike the space-containing bidix <r>
    # entries some sources leak (e.g. an "Aaron Copland" proper-noun pair with
...
def build_bidix(entries):
    ...
    for e in entries:
        ...
        # + insertion order) instead of the old order-dependent eo_terms[0]. The
        # winner must also be a term apertium-epo's monodix can generate — an
        # ungeneratable top pick (e.g. bert_embeddings' `trovis`, an inflected
        # surface form) is filtered out rather than shipped as a dead `#word`.
    ...
    for e in final_list_bidix:
        ...
        # including the generatability gate — this repeats it because pick_best
        # is a pure function of cands, so the same winner reproduces here).
        cands = _eo_candidates(e)
        if not cands:
            continue
        epo = pick_best(cands, valid=eo_generatable)
